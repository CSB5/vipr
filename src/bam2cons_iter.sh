...
# 
which bwa_unique.sh >/dev/null || exit 1
which samtools >/dev/null || exit 1
which bam2cons.py >/dev/null || exit 1
bam2cons.py -h >/dev/null
if [ $? -ne 0 ]; then
...
while [ $iter -le $MAX_ITER ]; do
    ...
    echo "INFO: calling bwa_unique.sh with the following args: $bwa_unique_args"
    #test $DEBUG -eq 1 && echo "DEBUG iter $iter: bwa_unique_args=$bwa_unique_args" 1>&2
    bwa_unique.sh $bwa_unique_args || exit 1
    ...
    test $DEBUG -eq 1 && echo "DEBUG iter $iter: calling bam2cons.py -b $bam -r $seq -o $new_ref --no-cov-char N" 1>&2
    bam2cons.py -b $bam -r $seq -o $new_ref --no-cov-char N -v || exit 1
